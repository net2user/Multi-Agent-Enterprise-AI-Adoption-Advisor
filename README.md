...
Complete and working end to end. Five agents, an orchestrator, and a Streamlit dashboard are built, tested, and confirmed against use cases spanning BFSI and Healthcare, plus a separate round of robustness testing against inputs the system was never designed for.
...
Use case input flows into the Orchestrator built on LangGraph, then sequentially through the Value agent, Risk and Governance agent, Architecture agent, and Adoption agent, then into the Executive Summary agent, producing a value score, risk score, complexity score, adoption score, and a board level recommendation of Fund, Fund with Conditions, Delay, or Do Not Fund. Separately, once multiple use cases are scored, the Portfolio Prioritization agent ranks them as Quick Win, Strategic Bet, Long Term Play, or Reconsider.
...
Beyond the three calibrated use cases above, the system was deliberately tested against inputs it was never designed for, to check whether it fabricates confident sounding answers when given weak or unsuitable input, a known failure mode for LLM systems that always try to produce a plausible looking output regardless of input quality.

Test one, "Deploy AI to pick my lunch order every day," correctly returned Do Not Fund with Value 10, Risk 10, Complexity 20, Adoption 20, all Low tier, and a rationale explicitly naming the lack of a real business case rather than fabricating one.

Test two, a near empty, vague input with no real detail, correctly returned Do Not Fund with Value 0, Risk 0, Complexity 0, Adoption 10, and a rationale stating outright that there was insufficient information to complete a meaningful assessment, rather than inventing detail to fill the schema.

Test three, retail inventory and staffing optimization, a coherent and well specified use case entirely outside the system's BFSI and Healthcare calibration, still returned a sensible Fund with Conditions recommendation with Value 75, Risk 40, Complexity 65, Adoption 68, reasoning about retail operations on their own terms rather than forcing awkward banking or healthcare language onto an unrelated sector.

In all three cases, the system responded with judgment rather than empty confidence, correctly distinguishing a genuine, well specified enterprise AI use case from one that is not, which matters as much as getting the calibrated use cases right.
...
Streamlit renders a dollar sign as the start of LaTeX math notation by default, which briefly caused dollar amounts in the Executive Summary output to render incorrectly on screen. The fix was escaping dollar signs before display, worth knowing for anyone displaying LLM generated financial figures inside a Streamlit app.
